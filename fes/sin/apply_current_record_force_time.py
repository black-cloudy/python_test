...
class apply_current_record_force:
    def __init__(self, current_min, current_max, channel):
        self.port = 'com15'
    ...
    def generate_current_list(self):
        while self.current_min < self.current_max:
            self.current_list.append(self.current_min)
            self.current_min += 0.01
    ...
    def fes_threading(self):
        ...
        while self.flag < len(self.current_list):
            ...
            # apply_current_stimulation(self.channel, 0)
            # print('通道、电流为：',self.channel, self.current_list[self.flag])
            time.sleep(0.01)
        ...
        # 指定.csv文件的路径和文件名
        csv_file = 'linear_current_force_2_7.csv'                     # 保存文件路径、名称    保存：电流、力、时间
...
def main():
    shili=apply_current_record_force(3, 14 ,4)                                                                           # 电流最小值、最大值、通道
    shili.generate_current_list()
    shili.plot_function()
    shili.init_force()
