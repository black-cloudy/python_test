...
# --------------------------------------------以下为电刺激串口通信
ser_current = serial.Serial(port='COM3', baudrate='115200')
...
class force2current:
    ...
    def generate_horizontal_line(self,y_value, num_points):
        # 创建一个x轴的值
        x = np.linspace(0, num_points - 1, num_points)
        # 创建一个y轴的值，所有点的y值都是相同的
        self.expect_force_waveform = np.full_like(x, y_value)
        return(len(self.expect_force_waveform))
...
def main():
    ...
    i=0
    # 计算食指拟合力的大小
    index_finger_force_calculate=hill_model_nihe('sin_current_force_1.csv')            # 存放电流、力、时间、肌电的文件  这是sin_current_force_1_1.2.3.4csv五个的平均值吧
    ...
    force_data_number = index_finger_force_mapping_current.generate_horizontal_line(15,10)
    ...
    while i<force_data_number:
        ...
        current=index_finger_force_mapping_current.find_closest_current()-3                                         # 疲劳程度带来的，所以增加几个电流
        closest_current_list_index_finger.append(current)
        i+=1
    # index_finger_force_mapping_current.expect_force_waveform = [x * 0.17 for x in index_finger_force_mapping_current.expect_force_waveform]    #优化电流大小
    ...
    force_data_number = thumb_finger_force_mapping_current.generate_horizontal_line(15, 10)
    ...
    while i < force_data_number:
        ...
        current = thumb_finger_force_mapping_current.find_closest_current()-6.7                                            # 疲劳程度带来的，所以增加几个电流
        closest_current_list_thumb_finger.append(current)
        i += 1
    # thumb_finger_force_mapping_current.expect_force_waveform = [x * 0.5 for x in thumb_finger_force_mapping_current.expect_force_waveform]  #优化电流大小
    ...
    force_device_1 = force_init('com3')                     # 力传感器初始化
    ser_device_1,init_ff_1=force_device_1.init_force()
    force_device_2= force_init('com3')
    ...
    while j<len(closest_current_list_thumb_finger):
        ...
        give_index_finger_current=give_current_and_record_force(6,closest_current_list_index_finger[j]-3+i*0.1,init_ff_1,ser_device_1)  ## 通道、电流、力传感器初始化的两个参数
        print('电流大小：',closest_current_list_index_finger[j]-3+i*0.1)
        verify_index_force=give_index_finger_current.two_threading_task()
        print('食指验证力：', verify_index_force)
        index_all_force.append(verify_index_force)

        give_thumb_finger_current=give_current_and_record_force(7,closest_current_list_thumb_finger[j]-3+i*0.1,init_ff_2,ser_device_2)  ## 通道、电流、力传感器初始化的两个参数
        verify_thumb_force=give_thumb_finger_current.two_threading_task()
        print('拇指验证力：', verify_thumb_force)
        thumb_all_force.append(verify_thumb_force)
        if(closest_current_list_index_finger[j]-3+i*0.1<closest_current_list_index_finger[j]):
            i=i+1
        # print('111111111111',len(thumb_all_force))
        # average_value_thumb = extract_and_average(give_thumb_finger_current.all_force)
        if len(thumb_all_force)>90:
            average_value_index = extract_and_average(index_all_force)
            verify_force_index_finger.append(average_value_index)
            average_value_thumb = extract_and_average(thumb_all_force)
            verify_force_thumb_finger.append(average_value_thumb)
            j=j+1
            i=0
            print("第一次采集结束")
            apply_current_stimulation(6, 0)
            time.sleep(0.01)
            apply_current_stimulation(7, 0)
            time.sleep(0.01)
            time.sleep(10)
            print("下一组开始")
    apply_current_stimulation(6, 0)
    ...
    # 将两个列表写入到CSV文件中
    with open('sandiantu15_8.csv', 'w', newline='') as file:                                                                   # 存放两个手指力的文件
        writer = csv.writer(file)

        # 使用 zip 将两个列表的对应元素配对
        for item1, item2,item3, item4 in zip(verify_force_index_finger, index_finger_force_mapping_current.expect_force_waveform,verify_force_thumb_finger, thumb_finger_force_mapping_current.expect_force_waveform):
